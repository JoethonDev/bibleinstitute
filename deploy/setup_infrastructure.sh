...
# Dozzle monitoring is intentionally separate: after TLS/Nginx is ready, run
# bash deploy/setup_monitoring.sh before the normal blue/green deployment.
set -Eeuo pipefail
...
write_monitoring_server_config() {
    [[ "$MONITORING_DOMAIN" =~ ^[A-Za-z0-9]([A-Za-z0-9.-]{0,251}[A-Za-z0-9])?$ ]] \
        || die "MONITORING_DOMAIN must be a hostname without a scheme, path, port, or Nginx syntax"

    local config_tmp="${RUNTIME_DIR}/nginx-monitoring-server.conf.tmp.$$"
    cat > "$config_tmp" <<NGINX
# Generated by deploy/setup_infrastructure.sh; do not edit manually.
# Exact host: ${MONITORING_DOMAIN}
server {
    ...
    # Dozzle is the only service behind this exact monitoring host.
    location / {
...
NGINX
    ...
    mv "$config_tmp" "$RUNTIME_DIR/nginx-monitoring-server.conf"
}
...
if [[ $# -gt 0 ]]; then
    case "$1" in
        -h|--help)
            ...
            echo "Dozzle monitoring is separate — run 'bash deploy/setup_monitoring.sh' after this."
            exit 0
            ;;
...
if [[ -r "$CERT_FILE" && -r "$KEY_FILE" ]]; then
    ...
    write_monitoring_server_config
    info "Generated Nginx monitoring host for https://${MONITORING_DOMAIN}/."
    ...
    docker compose --env-file "$COMPOSE_ENV_FILE" -f "$INFRA_COMPOSE" exec -T nginx nginx -s reload || die "Nginx reload FAILED"
...
if [[ -r "$CERT_FILE" && -r "$KEY_FILE" ]]; then
    info "  Running: nginx"
    info "  Monitoring host: https://${MONITORING_DOMAIN}/ (run setup_monitoring.sh to start Dozzle)"
else
...
echo "  2. Run: bash deploy/setup_monitoring.sh"
echo "  3. Run forward migrations and deploy the application"
echo ""
echo "  Monitoring is intentionally separate: deploy/setup_monitoring.sh generates the"
echo "  Dozzle users file and starts dozzle-socket-proxy + dozzle at"
echo "  https://${MONITORING_DOMAIN}/."
echo ""
